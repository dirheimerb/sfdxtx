...
export default class Oneview extends LightningElement {
  ...
  wiredRecord({ error, data }) {
    if (error) {
      ...
      this.contactSecondary_Email__c =
        this.contactData.fields.Secondary_Email__c.value;
      this.contactMobilePhone = this.contactData.fields.MobilePhone.value;
      this.contactData.fields.MailingStreet.length > 0
        ? (this.contactMailingStreet =
            this.contactData.fields.MailingStreet.value)
        : (this.contactMailingStreet = "");
      this.contactData.fields.MailingStateCode.length > 0
        ? (this.contactMailingStateCode =
            this.contactData.fields.MailingStateCode.value)
        : (this.contactMailingStateCode = "");
      this.contactData.fields.MailingPostalCode.length > 0
        ? (this.contactMailingPostalCode =
            this.contactData.fields.MailingPostalCode.value)
        : (this.contactMailingPostalCode = "");
      this.contactData.fields.MailingCountry.length > 0
        ? (this.contactMailingCountryCode =
            this.contactData.fields.MailingCountry.value)
        : (this.contactMailingCountryCode = "");
      this.contactData.fields.MailingCity.length > 0
        ? (this.contactMailingCity = this.contactData.fields.MailingCity.value)
        : (this.contactMailingCity = "");
      console.log("this.contactMailingCity", this.contactMailingCity);
      console.log(
        "this.contactMailingCountryCode",
        this.contactMailingCountryCode
      );
      console.log("contactEmail is", this.contactEmail);
      console.log("contactPhone is", this.contactPhone);
      console.log(
        "contactSecondary_Email__c is",
        this.contactSecondary_Email__c
      );
      console.log("contactMobilePhone is", this.contactMobilePhone);
      console.log("contactMailingCity is", this.contactMailingCity);
      console.log(
        "contactMailingCountryCode is",
        this.contactMailingCountryCode
      );
      console.log("contactMailingPostalCode is", this.contactMailingPostalCode);
      console.log("contactMailingStateCode is", this.contactMailingStateCode);
      console.log("contactMailingStreet is", this.contactMailingStreet);

      console.log(
        "this.contactData.fields.MailingCountry.value.length",
        this.contactData.fields.MailingCountry.value.length
      );
      console.log(
        "this.contactData.fields.MailingStateCode.length",
        this.contactData.fields.MailingStateCode.value.length
      );
    }
  }
  ...
  connectedCallback() {
    ...
    getUnifiedProfile({ recordId: this.recordId })
      .then((result) => {
        ...
        if (result.status === this.CONSTANT.ERROR_STATUS) {
          ...
          this.unifiedProfileDetailWrapper = JSON.parse(
            JSON.stringify(unifiedProfileData)
          ); /*SFSC-7001- Radiobutton disable*/
          ...
          if (this.unifiedProfileDetailWrapper.UPId === undefined) {
            ...
            if (
              this.unifiedProfileDetailWrapper.MobileContactPoints.length > 0
            ) {
              this.mobileContactPointsAvailable = true;
            }
            if (
              this.unifiedProfileDetailWrapper.PhoneContactPoints.length > 0
            ) {
              this.phoneContactPointsAvailable = true;
            }
            if (
              this.unifiedProfileDetailWrapper.EmailContactPoints.length > 0
            ) {
              this.emailContactPointsAvailable = true;
            }
            if (
              this.unifiedProfileDetailWrapper.SecondaryEmailContactPoints
                .length > 0
            ) {
              this.secondaryEmailContactPointsAvailable = true;
            }
            if (
              this.unifiedProfileDetailWrapper.AddressContactPoints.length > 0
            ) {
              this.addressContactPointsAvailable = true;
            }
            /*SFSC-7001- Radiobutton disable - starts*/
            var emailContactPoints =
            ...
            var secondaryEmailContactPoints =
              this.unifiedProfileDetailWrapper.SecondaryEmailContactPoints;
            secondaryEmailContactPoints.forEach((profileSecEmail) => {
              if (
                profileSecEmail.contactPointValue ===
                this.contactSecondary_Email__c
              ) {
            ...
            var addressContactPoints =
              this.unifiedProfileDetailWrapper.AddressContactPoints;
            addressContactPoints.forEach((profileAddress) => {
              if (
                profileAddress.addressLine1 === this.contactMailingStreet &&
                profileAddress.cityName === this.contactMailingCity &&
                profileAddress.stateProvinceName ===
                  this.contactMailingStateCode &&
                profileAddress.countryName === this.contactMailingCountryCode &&
                profileAddress.postalCode === this.contactMailingPostalCode
              ) {
      ...
      .catch((error) => {
        ...
        this.errorMessage =
          "There is some error in finding data for this record, Contact your administrator!!";
      });
  }
  ...
  adoptContactDetails() {
    ...
    var adoptedValuesJSON = {
      ...
      adoptedSecondaryEmailValues:
        adoptedSecondaryEmailValue === "" ? null : adoptedSecondaryEmailValue,
