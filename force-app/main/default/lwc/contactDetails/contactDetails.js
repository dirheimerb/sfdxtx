...
export default class ContactDetails extends NavigationMixin(LightningElement) {
    ...
    get isPhysicianIndependenceDisabled() {
        return (!this.physicianIndependenceOptions || !this.physicianIndependenceOptions.length);
    }
//Testing
    ...
    recordInfo({ data, error }) {
        if (data) {
            ...
            this.layoutSections = objData.layouts.Contact[recordTypeId].Full.View.sections;
            this.recordData = objData;
            this.contactRecordData = data.records[this.recordId].fields;
            this.updatedAddressDetails.Job_Function__c = (this.contactRecordData.Job_Function__c != null) ? this.contactRecordData.Job_Function__c.value : '';
            this.updatedAddressDetails.Physician_Independence__c = (this.contactRecordData.Physician_Independence__c != null) ? this.contactRecordData.Physician_Independence__c.value : '';
            ...
            for (var i = 0; i < this.layoutSections.length; i++) {
                ...
                if (currSection.useHeading == true) {
                    var incr = i + 1;
                    while (this.layoutSections[incr] != null && this.layoutSections[incr].useHeading == false) {
                    ...
                    r.layoutItems.forEach(i => {
                        ...
                        if (s.heading == this.LABEL.SYSTEM_INFORMATION) {
                            ...
                            if (i.label == this.LABEL.CREATED_BY) {
                                ...
                                i.isRecordType = true;
                            } else if (i.label != this.LABEL.CREATED_BY && i.label != this.LABEL.LAST_MODIFIED_BY && i.label != this.LABEL.CONTACT_RECORD_TYPE && i.label != '') {
    ...
    @wire(getPicklistValues, { recordTypeId: "$recordTypeId", fieldApiName: MAILING_COUNTRY_FIELD })
    ...
    @wire(getPicklistValues, { recordTypeId: "$recordTypeId", fieldApiName: MAILING_STATE_FIELD })
    ...
    handleJobFunctionChange(event) {
        ...
        let key = this.allPhysicianIndependence.controllerValues[event.target.value];
        let filteredPhysicianIndependence = this.allPhysicianIndependence.values.filter(opt => opt.validFor.includes(key));
        this.physicianIndependenceOptions = (filteredPhysicianIndependence.length ? [DEFAULT_PICKLIST_VALUE, ...filteredPhysicianIndependence] : []);
    }

    addressChanged(event) {
        this.isAddressChanged = true;
        this.updatedAddressDetails.MailingStreet = (event.target.street == null) ? '' : event.target.street;
        this.updatedAddressDetails.MailingCity = (event.target.city == null) ? '' : event.target.city;
        this.updatedAddressDetails.MailingPostalCode = (event.target.postalCode == null) ? '' : event.target.postalCode;
        this.updatedAddressDetails.MailingStateCode = (event.target.province == null) ? '' : event.target.province;
        this.updatedAddressDetails.MailingCountryCode = (event.target.country == null) ? '' : event.target.country;
    ...
    handleCountryChange(countryName) {
        ...
        let filteredStates = this.allMailingStates.values.filter(opt => opt.validFor.includes(key));
        this.mailingStates = [...filteredStates];
    }

    handleFieldChange(event) {//not needed
    ...
    editButton(event) {
        ...
        let key = this.allMailingStates.controllerValues[this.contactRecordData.MailingCountryCode.value];
        let filteredStates = this.allMailingStates.values.filter(opt => opt.validFor.includes(key));
        this.mailingStates = [...filteredStates];
    ...
    validateDQServices() {
        ...
        try {
                ...
                .then(result =>{
                    ...
                    DQMetetadataRecords.forEach( t=> { 
                        if(t.DeveloperName == 'Email_Validation'){
                            ...
                            if (email != null && email != '' && email != this.contactRecordData.Email.value && t.DQ_Service_On__c) {
                            ...
                            if (secondaryEmail != null && secondaryEmail != '' && secondaryEmail != this.contactRecordData.Secondary_Email__c.value && t.DQ_Service_On__c) {
                                ...
                                if (this.isEmptyOrNull(secondaryEmail)) {
                                    this.updatedContactDetail.Secondary_Email_Verification_Status__c = null;
                                    this.updatedContactDetail.Secondary_Email_Verification_Date__c = null;
                                }
                            }
                        }else if(t.DeveloperName == 'Phone_Validation'){
                             //validate phone   
                            if (phone != null && phone != '' && phone != this.contactRecordData.Phone.value && t.DQ_Service_On__c) {
                            ...
                            if (mobilePhone != null && mobilePhone != '' && mobilePhone != this.contactRecordData.MobilePhone.value && t.DQ_Service_On__c) {
                                this.DQValidationPhone(mobilePhone, 'isMobile');
                            } else {
                                this.isMobileValidationCallbackReceived = true;
                                if (this.isEmptyOrNull(mobilePhone)) {
                                    this.updatedContactDetail.Mobile_Phone_Verification_Status__c = null;
                                    this.updatedContactDetail.Mobile_Phone_Verification_Date__c = null;
                                }
                            }

                        }else if(t.DeveloperName == 'Address_Validation'){
                            let oldAddress = (this.getEmptyStringForNullValue(this.contactRecordData.MailingStreet.value))
                            + (this.getEmptyStringForNullValue(this.contactRecordData.MailingCity.value))
                            + (this.getEmptyStringForNullValue(this.contactRecordData.MailingCountryCode.value))
                            + (this.getEmptyStringForNullValue(this.contactRecordData.MailingStateCode.value))
                            + (this.getEmptyStringForNullValue(this.contactRecordData.MailingPostalCode.value));
                            let newAddress = (this.getEmptyStringForNullValue(this.updatedContactDetail.MailingStreet))
                            + (this.getEmptyStringForNullValue(this.updatedContactDetail.MailingCity))
                            + (this.getEmptyStringForNullValue(this.updatedContactDetail.MailingCountryCode))
                            + (this.getEmptyStringForNullValue(this.updatedContactDetail.MailingStateCode))
                            + (this.getEmptyStringForNullValue(this.updatedContactDetail.MailingPostalCode));


                            if (this.isAddressChanged && oldAddress != newAddress && !this.isEmptyOrNull(newAddress)
            ...
            let checkAllResponse = window.setInterval(() => {
                if (this.isMobileValidationCallbackReceived && this.isPhoneValidationCallbackReceived && this.isEmailValidationCallbackReceived && this.isSecondaryEmailValidationCallbackReceived && this.isAddressValidationCallbackReceived) {
                    this.isSpinner = true;
                    //show Success Message - starts               
                    if (this.showSuccessMsgAddress == this.LABEL.Address && this.showSuccessMsgEmail == this.LABEL.Email && this.showSuccessMsgPhone == this.LABEL.Phone && this.showSuccessMsgSecondaryEmail == this.LABEL.SecondaryEmail && this.showSuccessMsgMobilePhone == this.LABEL.Mobile) {
                        this.saveData();
                        this.inValidData = 'DQ_CALLOUT';
                        //this.isSpinner = false;
                    } else if (this.showSuccessMsgPhone == this.LABEL.invalidPhone || this.showSuccessMsgMobilePhone == this.LABEL.invalidMobile || this.showSuccessMsgEmail == this.LABEL.invalidEmail || this.showSuccessMsgSecondaryEmail == this.LABEL.invalidSecondaryEmail || this.showSuccessMsgAddress == this.LABEL.invalidAddress) {//to stop spinner when invalid modal is displayed and save invalid data
                        ...
                        this.isSpinner = false;
                    } else if (this.showSuccessMsgAddress == 'exceptionAddress' || this.showSuccessMsgEmail == 'exceptionEmail' || this.showSuccessMsgSecondaryEmail == 'exceptionSecondaryEmail' || this.showSuccessMsgPhone == 'exceptionPhone' || this.showSuccessMsgMobilePhone == 'exceptionMobilePhone') { //handle if DQ API has exception
        ...
        DQValidateEmail({ emailName: email, recordId: null })
            .then(result => {
                ...
                if (emailCheck == 'isPrimaryEmail') {
                    ...
                    if (emailResponss.exceptionMessage == this.LABEL.Read_timed_out || emailResponss.exceptionMessage != null) {
    ...
    DQVerifyPrimaryEmail(emailResponss) {
        ...
        this.updatedContactDetail.Email_Verification_Status__c = emailResponss.statusDescription;
        this.updatedContactDetail.Email_Verification_Date__c = today;

        console.log('DQVerifyPrimaryEmail: ', this.updatedContactDetail.Email_Verification_Status__c, this.updatedContactDetail.Email_Verification_Date__c)

        if (emailResponss.statusDescription == this.LABEL.Verified_Valid || emailResponss.statusDescription == this.LABEL.Verified_Questionable
    ...
    DQVerifySecondaryEmail(emailResponss) {
        var today = this.calculateTodaysDate();
        this.updatedContactDetail.Secondary_Email_Verification_Status__c = emailResponss.statusDescription;
        this.updatedContactDetail.Secondary_Email_Verification_Date__c = today;
        console.log('DQVerifySecondaryEmail : ', this.updatedContactDetail.Secondary_Email_Verification_Status__c, this.updatedContactDetail.Secondary_Email_Verification_Date__c);
        if (emailResponss.statusDescription == this.LABEL.Verified_Valid || emailResponss.statusDescription == this.LABEL.Verified_Questionable
            || emailResponss.statusDescription == this.LABEL.Unable_to_Process) {
    ...
    DQValidationPhone(phone, phoneCheck) {
        //this.inValidData = 'DQ_CALLOUT';
        DQValidatePhone({ phoneNumber: phone, recordId: this.recordId, fromLocation: 'isEdit', MailingCountryCode: this.updatedContactDetail.MailingCountryCode })
            .then(result => {
                ...
                if (phoneCheck == 'isPhone') {
                    ...
                    //handle exception - set status and date as null when exception is received
                    if (phoneResponss.exceptionMessage == this.LABEL.Read_timed_out || phoneResponss.exceptionMessage != null) {
    ...
    DQVerifyPhone(phoneResponss) {
        var today = this.calculateTodaysDate();
        this.updatedContactDetail.Phone_Verification_Status__c = phoneResponss.statusDescription;
        this.updatedContactDetail.Phone_Verification_Date__c = today;
        if ((phoneResponss.statusDescription == this.LABEL.Verified_Valid || phoneResponss.statusDescription == this.LABEL.Unable_to_Process)) {
    ...
    DQVerifyMobilePhone(phoneResponss) {
        var today = this.calculateTodaysDate();
        this.updatedContactDetail.Mobile_Phone_Verification_Status__c = phoneResponss.statusDescription;
    ...
    DQValidationAddress() {
        ...
        if (this.updatedContactDetail.MailingStreet && this.updatedContactDetail.MailingStreet != '') {
        ...
        DQValidateAddress({
            ...
            cityName: (this.updatedContactDetail.MailingCity == undefined ? '' : this.updatedContactDetail.MailingCity),
            stateName: (this.updatedContactDetail.MailingStateCode == undefined ? '' : this.updatedContactDetail.MailingStateCode),
            zipCode: (this.updatedContactDetail.MailingPostalCode == undefined ? '' : this.updatedContactDetail.MailingPostalCode),
            countryName: (this.updatedContactDetail.MailingCountryCode == undefined ? '' : this.updatedContactDetail.MailingCountryCode),
            timeOut: null
        })
            .then(result => {
                if (result.status.includes('Error')) {
                    ...
                    var today = this.calculateTodaysDate();
                    this.updatedContactDetail.Address_Verification_Status__c = result.status;
    ...
    saveData() {
        ...
        console.log('Save_Data : ', JSON.parse(JSON.stringify(this.updatedContactDetail)));
        this.template.querySelector('lightning-record-edit-form').submit(this.updatedContactDetail);
    }
    ...
    updateContact(data) {
        ...
        duplicateContact['Email_Verification_Status__c'] = this.updatedContactDetail.hasOwnProperty('Email_Verification_Status__c') ? this.updatedContactDetail.Email_Verification_Status__c : duplicateContact.Email_Verification_Status__c;
        duplicateContact['Email_Verification_Date__c'] = this.updatedContactDetail.hasOwnProperty('Email_Verification_Date__c') && this.updatedContactDetail.Email_Verification_Date__c ? this.formatDate(this.updatedContactDetail.Email_Verification_Date__c) : duplicateContact.Email_Verification_Date__c;
        duplicateContact['Secondary_Email_Verification_Status__c'] = this.updatedContactDetail.hasOwnProperty('Secondary_Email_Verification_Status__c') ? this.updatedContactDetail.Secondary_Email_Verification_Status__c : duplicateContact.Secondary_Email_Verification_Status__c;
        duplicateContact['Secondary_Email_Verification_Date__c'] = this.updatedContactDetail.hasOwnProperty('Secondary_Email_Verification_Date__c') && this.updatedContactDetail.Secondary_Email_Verification_Date__c ? this.formatDate(this.updatedContactDetail.Secondary_Email_Verification_Date__c) : duplicateContact.Secondary_Email_Verification_Date__c;
        duplicateContact['Phone_Verification_Status__c'] = this.updatedContactDetail.hasOwnProperty('Phone_Verification_Status__c') ? this.updatedContactDetail.Phone_Verification_Status__c : duplicateContact.Phone_Verification_Status__c;
        duplicateContact['Phone_Verification_Date__c'] = this.updatedContactDetail.hasOwnProperty('Phone_Verification_Date__c') && this.updatedContactDetail.Phone_Verification_Date__c ? this.formatDate(this.updatedContactDetail.Phone_Verification_Date__c) : duplicateContact.Phone_Verification_Date__c;
        duplicateContact['Mobile_Phone_Verification_Status__c'] = this.updatedContactDetail.hasOwnProperty('Mobile_Phone_Verification_Status__c') ? this.updatedContactDetail.Mobile_Phone_Verification_Status__c : duplicateContact.Mobile_Phone_Verification_Status__c;
        duplicateContact['Mobile_Phone_Verification_Date__c'] = this.updatedContactDetail.hasOwnProperty('Mobile_Phone_Verification_Date__c') && this.updatedContactDetail.Mobile_Phone_Verification_Date__c ? this.formatDate(this.updatedContactDetail.Mobile_Phone_Verification_Date__c) : duplicateContact.Mobile_Phone_Verification_Date__c;
        duplicateContact['Address_Verification_Status__c'] = this.updatedContactDetail.hasOwnProperty('Address_Verification_Status__c') ? this.updatedContactDetail.Address_Verification_Status__c : duplicateContact.Address_Verification_Status__c;
        duplicateContact['Address_Verification_Date__c'] = this.updatedContactDetail.hasOwnProperty('Address_Verification_Date__c') && this.updatedContactDetail.Address_Verification_Date__c ? this.formatDate(this.updatedContactDetail.Address_Verification_Date__c) : duplicateContact.Address_Verification_Date__c;

        console.log('duplicateContact2 : ', JSON.parse(JSON.stringify(duplicateContact)));

        updateDuplicateContact({ duplicateContact }).then(result => {
    ...
    handleError(event) {
        ...
        if (formErrorDetails.hasOwnProperty('output')) {
            if (formErrorDetails.output.hasOwnProperty('fieldErrors')) {
                Object.entries(formErrorDetails.output.fieldErrors).forEach(([key, value]) => {
                    value.forEach(obj => {
                        errors.push(`${obj.fieldLabel}: ${obj.message}`);
                    });
                });
            }
            if (formErrorDetails.output.hasOwnProperty('errors') && formErrorDetails.output.errors.length) {
    ...
    get popoverHeaderStyle() {
        return (this.errorMessages.length ? 'slds-popover slds-popover_error slds-nubbin_bottom-left' : 'slds-popover slds-popover_warning  slds-nubbin_bottom-left') + ' popover-custom-width';
    }
    editRecordType() {
    ...
    validateFields(event) {
        ...
        try {
            ...
            [...this.template.querySelectorAll('lightning-input-field')].forEach(element => {
                ...
                if ((element.required && (element.value == null || element.value == '')) || !element.reportValidity()) {
                    if (this.contactInfo && this.contactInfo.data && this.contactInfo.data.fields.hasOwnProperty(element.fieldName) && this.contactInfo.data.fields[element.fieldName].dataType.toLowerCase() != 'boolean') {
                        allInputsAreFilled = false;
                        let fieldLabel = this.contactInfo.data.fields[element.fieldName].label;
