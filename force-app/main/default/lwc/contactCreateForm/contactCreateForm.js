...
export default class ContactCreateForm extends NavigationMixin(
  ...
  handleOnSubmit(whichButtonClicked) {
    if (whichButtonClicked == "Save") {
      ...
      this.whichButtonWasClicked = this._WhichButtonWasLastClicked =
        "SaveAndNew"; // Updated on 18 Jan 2022 - SFSC - 6173
    ...
    insertDuplicateContact({ duplicateContact })
      .then((result) => {
        ...
        if (result.isSuccuss) {
          ...
          if (
            this.whichButtonWasClicked &&
            (this.whichButtonWasClicked == "SaveAndNew" ||
              this._WhichButtonWasLastClicked == "SaveAndNew")
          ) {
...
handleReset() {
  ...
  closeModal() {
    ...
    else if (this.isComingFrom360SearchPage) {
      // Added on 4 Feb 2022
      this[NavigationMixin.Navigate]({
        type: "standard__navItemPage",
        attributes: {
